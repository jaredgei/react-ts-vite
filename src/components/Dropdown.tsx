...
const Dropdown = ({ title, value, content, options, customButton, anchorPosition = 'top left', isActive, hasError }: Props) => {
  ...
  return (
    <div
      ...
      {!customButton && <div className={'dropdownContainer' + (isActive ? ' active' : '') + (hasError ? ' error' : '')} onClick={expand}>
        <div className='dropdownTitle'>
          {/* render all options hidden so width scales to max */}
          <div className={'hiddenTitleOption'}>{title}</div>
          {options?.map((option: Option) => <div key={option.name} className={'hiddenTitleOption'}>{option.name}</div>)}
          <div className='dropdownTitleValue'>{value || title}</div>
        </div>
        {caret}
      </div>}
      {customButton && <div className='dropdownContainerCustom' onClick={expand}>{customButton}</div>}
      <div
        className='popup'
        style={popupStyle}
        ref={popup}>
        <Suggestions
          content={content}
          suggestions={workingOptions.map(option => option.name ? {
            name: option.name,
